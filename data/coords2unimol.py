...
class Coords2Unimol(object):
    ...
    def get(self, atoms, coordinates, max_atoms=256, **params):
        ...
        # src_distance[src_distance>6.0] = -1
        ### edge type 
    ...
    def get_tensor(self, atoms, coordinates, max_atoms=512, **params):
        ...
        # src_distance = np.zeros((len(src_tokens), len(src_tokens)))
        ### coordinates normalize & padding
        src_coord = coordinates - torch.mean(coordinates,dim=0)
        device = src_coord.device
        src_coord = torch.concatenate([torch.zeros((1,3)).to(device), src_coord, torch.zeros((1,3)).to(device)], axis=0)
        ### distance matrix
        src_distance = torch.cdist(src_coord, src_coord)
        # src_distance[src_distance>6.0] = -1
        ### edge type 
        src_edge_type = src_tokens.reshape(-1, 1) * len(self.dictionary) + src_tokens.reshape(1, -1)
    ...
    def get_tensors(self, atoms, coordinates, max_atoms=512, **params):
        ...
        ### coordinates normalize & padding
        # src_coord = coordinates - torch.mean(coordinates,dim=0)
        src_coord = coordinates - torch.mean(coordinates,dim=1).repeat(1,coordinates.shape[1]).reshape(coordinates.shape[0],-1,3)
        device = src_coord.device
        # src_coord = torch.concatenate([torch.zeros((1,3)).to(device), src_coord, torch.zeros((1,3)).to(device)], axis=0)
        src_coord = torch.concatenate([torch.zeros((src_coord.shape[0],1,3)).to(device), src_coord, torch.zeros((src_coord.shape[0],1,3)).to(device)], axis=1)
        ### distance matrix
        src_distance = torch.cdist(src_coord, src_coord)
        # src_distance[src_distance>6.0] = -1
        ### edge type 
        src_edge_type = src_tokens.reshape(-1, 1) * len(self.dictionary) + src_tokens.reshape(1, -1)
        src_tokens = torch.tensor(src_tokens).repeat(coordinates.shape[0],1)
        src_edge_type = torch.tensor(src_edge_type).repeat(coordinates.shape[0],1,1)

        return {
                'src_tokens': src_tokens.long(), 
                'src_distance': src_distance.float(), 
                'src_coord': src_coord.float(), 
                'src_edge_type': src_edge_type.long(),
                }
    
    def get_ads_tensors(self, atoms, coordinates, max_atoms=512, **params):
